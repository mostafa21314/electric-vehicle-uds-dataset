...
```
## Note: Raw data not included in this fork

This fork excludes `data/uds_data/` (the raw per-vehicle Parquet logs, ~800 MB via Git LFS
upstream) to keep the repository lightweight. To reproduce the full pipeline on a new
machine:

1. Download `data/uds_data/*.parquet` from the upstream dataset repo,
   [TUMFTM/electric-vehicle-uds-dataset](https://github.com/TUMFTM/electric-vehicle-uds-dataset)
   (download as .zip, or clone with Git LFS installed), and place them under `data/uds_data/`.
2. Run the pipeline in [`preprocessing/02_trip_timeseries`](preprocessing/02_trip_timeseries)
   to regenerate `data/processed/`.
